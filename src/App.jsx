...
const PieChartComponent = ({ data, total }) => {
    ...
    if (total === 0) return (
        <div className="w-32 h-32 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-400">无数据</div>
    );
    return (
...
const WealthJarView = ({ balances, setBalances, wealthConfig, setWealthConfig, transactions = [], setTransactions }) => {
    ...
    const [expenseForm, setExpenseForm] = useState({ amount: '', category: '饮食', remark: '', date: getLocalDateString(new Date()) });
    ...
    const handleDistribute = (e) => {
        ...
        if (wealthConfig.showCommitment && commit > 0) {
            ...
            newTransactions.unshift({ id: Date.now() + 1, amount: -commit, category: '固定开销扣除', remark: '自动扣除', date: getLocalDateString(new Date()), type: 'expense' });
        }
    ...
    const addTransaction = (e) => {
        ...
        setExpenseForm({ amount: '', category: '饮食', remark: '', date: getLocalDateString(new Date()) });
    };
    ...
    // FIX: Scan all jars that match "Savings" or "Investment" labels
        ...
        <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-24">
            <div className="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl">
                <div className="relative z-10 flex justify-between items-end">
                    ...
                    <button onClick={() => { const n = prompt("新目标金额:", wealthConfig.yearlyTarget); if(n) setWealthConfig({...wealthConfig, yearlyTarget: parseFloat(n)}); }} className="bg-white/10 px-4 py-2 rounded-xl text-sm font-bold">编辑目标</button>
                </div>
                <div className="mt-6 w-full bg-white/10 rounded-full h-2">
                    <div className="bg-emerald-400 h-2 rounded-full transition-all" style={{ width: `${Math.min(100, (savingsPlusInvestment / wealthConfig.yearlyTarget) * 100)}%` }}></div>
                </div>
            </div>
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                ...
                <form onSubmit={handleDistribute} className="flex flex-col md:flex-row gap-4">
                    ...
                    <button type="submit" className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-700">全部分配</button>
                </form>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {wealthConfig.showCommitment ? (
                    <div className="bg-rose-50 border border-rose-100 p-6 rounded-3xl flex flex-col justify-between h-40 relative group">
                        <div className="flex justify-between items-start">
                            <div className="font-bold text-rose-700">固定开销</div>
                ...
                <button onClick={() => setIsAddJarOpen(true)} className="border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center h-40 text-slate-400 hover:border-violet-400 font-bold gap-2"><Plus size={24}/> 添加存钱罐</button>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-6 shadow-sm h-fit">
                    <h3 className="font-bold text-slate-800 mb-4">记录日常开支</h3>
                    <form onSubmit={addTransaction} className="space-y-4">
                        ...
                        <input type="text" placeholder="类别 (如: 饮食)" value={expenseForm.category} onChange={e=>setExpenseForm({...expenseForm, category: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none" />
                        ...
                        <button type="submit" className="w-full bg-slate-900 text-white font-bold py-3 rounded-xl hover:bg-slate-700">录入支出</button>
                    </form>
                </div>
                <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 p-6 shadow-sm min-h-[400px]">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="font-bold text-slate-800">近期交易</h3>
                            <div className="text-xs font-bold text-slate-400 mt-1">净余额: <span className={netTransactionTotal >= 0 ? 'text-emerald-500' : 'text-rose-500'}>{netTransactionTotal >= 0 ? '+' : ''} RM {netTransactionTotal.toLocaleString()}</span></div>
                        </div>
                        <button onClick={() => setShowGraph(!showGraph)} className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 flex items-center gap-2"><PieChart size={14}/> {showGraph ? '列表' : '图表'}</button>
                    </div>
                    {showGraph ? (
                        <div className="flex flex-col items-center py-8">
                            <PieChartComponent data={graphData} total={totalExpense} />
                            <div className="mt-4 text-xl font-black">总支出: RM {totalExpense.toLocaleString()}</div>
                        </div>
                    ) : (
                        <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                            {transactions.length === 0 ? <div className="text-center text-slate-400 py-10">暂无记录。</div> : transactions.map(tx => (
            ...
            {isAddJarOpen && (
                <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
                        <h3 className="font-bold text-lg mb-4">添加新存钱罐</h3>
                        <form onSubmit={handleAddJar} className="space-y-4">
                            <input className="w-full border border-slate-200 rounded-xl p-2 outline-none focus:border-violet-500" value={newJarForm.label} onChange={e => setNewJarForm({...newJarForm, label: e.target.value})} placeholder="罐子名称 (如: 长期储蓄)" autoFocus />
                            <input type="number" className="w-full border border-slate-200 rounded-xl p-2 outline-none focus:border-violet-500" value={newJarForm.percent} onChange={e => setNewJarForm({...newJarForm, percent: e.target.value})} placeholder="分配比例 %" />
                            <div className="flex gap-2"><button type="button" onClick={() => setIsAddJarOpen(false)} className="flex-1 py-2 text-slate-500">取消</button><button type="submit" className="flex-1 py-2 bg-slate-900 text-white rounded-xl font-bold">创建</button></div>
                        </form>
                    </div>
                </div>
            )}
...
// FIX: Dedicated Row component to prevent IME jumping by buffering state locally
const CycleTaskRow = memo(({ task, cycleId, onUpdate, onDelete }) => {
    ...
    // Sync from parent only if task object actually changes (data refresh)
    useEffect(() => {
...
const CycleTrackerView = ({ data, setData, startYearDate, setStartYearDate }) => {
    ...
    const addTask = (cycleId) => {
        setData(prev => prev.map(cycle => {
            ...
            if ((cycle.tasks || []).length >= 5) { alert("每10天周期最多5个任务！"); return cycle; }
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in pb-20">
            <div className="flex justify-between items-end mb-6 sticky top-0 bg-slate-50 z-20 py-2 border-b border-slate-200">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800 flex items-center gap-3"><Activity className="text-violet-600" /> 36 x 10 周期追踪</h2>
                    <div className="flex items-center gap-4 mt-2">
                        <div className="bg-white border border-slate-200 px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-2">
                            <span className="text-xs font-bold text-slate-400 uppercase">起始日</span>
                ...
                <div className="w-1/3">
                    <div className="flex justify-between text-xs font-bold mb-1"><span>全年进度</span><span className="text-violet-600">{progress}%</span></div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                <div className="grid grid-cols-12 gap-4 px-6 mb-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    ...
                    <div className="col-span-3">执行计划</div>
                            ...
                            <div className="grid grid-cols-12 gap-6">
                                ...
                                <div className="col-span-10 space-y-3">
                                    {cycle.tasks && cycle.tasks.length > 0 ? (
                                        ...
                                        <div className="h-full flex items-center justify-center text-slate-300 text-xs italic font-medium">本周期暂无任务。点击左侧加号开始。</div>
                                    )}
                                </div>
                            </div>
                        </div>
    ...
    <div className="flex h-screen w-full bg-slate-50 font-sans text-slate-800 overflow-hidden">
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-100 shadow-2xl md:shadow-none transform transition-transform duration-300 md:translate-x-0 md:static flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8">
          ...
          <nav className="space-y-1.5">{[{ id: 'focus', label: '仪表盘', icon: Home }, { id: 'wealth', label: '存钱罐', icon: Database }, { id: 'calendar', label: '日历', icon: CalIcon }, { id: 'kanban', label: '焦点轴', icon: Trello }, { id: 'cycle', label: '36x10 周期', icon: Activity }].map(item => (
