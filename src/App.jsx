...
const DashboardView = ({ tasks, onAddTask, user, openAddModal, toggleTask, deleteTask, categoryColors }) => {
    ...
    // --- JARVIS LOGIC V2: TIME BLOCKING ORCHESTRATOR ---
    const handleJarvisPlan = (e) => {
        ...
        setTimeout(() => {
            ...
            let eventName = aiInput
                .replace(timeRegex, '')
                .replace(/tomorrow|today|明天|今天|arrive|start|at|by/gi, '')
                .trim() || "Event";
            ...
            if (anchorHour >= 12) {
                tasksToAdd.push({ id: generateId(), title: `🧠 Deep Work: Priority Project`, time: "09:00", category: '工作', date: targetDate });
            }

            const isLunchBusy = (anchorHour === 12) || (travelHour === 12);
            if (!isLunchBusy) {
                tasksToAdd.push({ id: generateId(), title: `🍱 Lunch Break`, time: "12:30", category: '生活', date: targetDate });
            }
            ...
            const responseText = `Sir, I've orchestrated your entire day around ${eventName}. Wake up is set for ${wakeTimeStr}. Departure is at ${travelTimeStr} to beat KL traffic. I've filled your gaps with Deep Work and Gym sessions to maximize productivity.`;
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-24">
        <header>
            ...
            <p className="text-slate-500 font-medium">Welcome back, <span className="text-violet-600">{user ? user.email.split('@')[0] : 'Guest'}</span></p>
        </header>
        ...
        <div className="bg-slate-900 rounded-3xl p-6 text-white shadow-xl shadow-slate-200 relative overflow-hidden">
            ...
            <div className="flex items-start gap-4 relative z-10">
                <div className="bg-cyan-900/50 p-3 rounded-2xl border border-cyan-500/30">
                ...
                <div className="flex-1">
                    ...
                    <form onSubmit={handleJarvisPlan} className="flex gap-3">
                        <input 
                            ...
                            placeholder='e.g. "Tomorrow 9:30am arrive ING LIVE KL"'
                            className="flex-1 bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 placeholder-slate-500 text-white outline-none focus:border-cyan-500 transition-all font-medium text-sm"
                        />
                        <button 
                            type="submit" 
                            disabled={isProcessing}
                            className="bg-cyan-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-cyan-500 transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 min-w-[120px] justify-center"
                        >
                            {isProcessing ? <Activity className="animate-spin" size={18}/> : <Navigation size={18} fill="currentColor"/>}
                            {isProcessing ? 'PLANNING' : 'EXECUTE'}
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm p-6 min-h-[400px]">
                ...
                <div className="space-y-2">
                    {todaysTasks.length === 0 ? (
                        <div className="text-center text-slate-400 py-10">No tasks for today.</div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                ...
                <div className="space-y-4">
                    {Object.keys(catStats).length === 0 && <p className="text-xs text-slate-400">Add tasks to see analysis.</p>}
                    {Object.entries(catStats).map(([cat, stat]) => (
                        <div key={cat}>
                            ...
                            <div className="w-full bg-slate-100 rounded-full h-2">
                                <div className="bg-blue-500 h-2 rounded-full transition-all duration-500" style={{width: `${(stat.completed/stat.total)*100}%`}}></div>
                            </div>
                        </div>
                    ))}
                </div>
...
const WealthJarView = ({ balances, setBalances, wealthConfig, setWealthConfig, transactions = [], setTransactions }) => {
    ...
    const deleteCommitment = () => {
        ...
        if(balance > 0) {
            ...
            if(targetJar) {
                 ...
                 alert("Target jar not found. Deletion cancelled.");
                 return;
            }
        }
        // Disable commitment logic
        setWealthConfig({ ...wealthConfig, showCommitment: false, commitment: 0 });
    };

    const deleteJar = (id) => {
        if(wealthConfig.jars.length < 1 && !wealthConfig.showCommitment) {
             // Allow deleting down to 0 if needed, but warning provided
        }
    ...
    const handleCommitmentChange = (e) => {
        ...
        setWealthConfig({ ...wealthConfig, commitment: isNaN(val) ? 0 : val });
    };

    const netTransactionTotal = transactions.reduce((acc, tx) => acc + tx.amount, 0);
        ...
        <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-24">
                ...
                <div className="relative z-10 flex justify-between items-end">
                    <div>
                        ...
                        <div className="text-4xl font-black">
                            RM {((balances.savings||0) + (balances.investment||0)).toLocaleString()} 
                            <span className="text-slate-500 text-2xl font-bold"> / {wealthConfig.yearlyTarget.toLocaleString()}</span>
                        </div>
                    </div>
                    <button onClick={() => {
                        const newTarget = prompt("New Target Amount:", wealthConfig.yearlyTarget);
                        if(newTarget) setWealthConfig({...wealthConfig, yearlyTarget: parseFloat(newTarget)});
                    }} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-sm font-bold transition-colors">Edit Target</button>
                </div>
                <div className="mt-6 w-full bg-white/10 rounded-full h-2">
                    <div className="bg-emerald-400 h-2 rounded-full" style={{ width: `${Math.min(100, (((balances.savings||0) + (balances.investment||0)) / wealthConfig.yearlyTarget) * 100)}%` }}></div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                ...
                <form onSubmit={handleDistribute} className="flex flex-col md:flex-row gap-4">
                    <input type="number" placeholder="Enter Income (RM)" value={income} onChange={e=>setIncome(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-emerald-500 font-bold text-lg"/>
                    
                    {/* Fixed Commitment - Condition Render */}
                    {wealthConfig.showCommitment && (
                        <div className="flex items-center gap-2 px-4 py-3 bg-rose-50 rounded-xl border border-rose-100 text-rose-600 font-bold min-w-[200px]">
                            ...
                            <input 
                                type="number" 
                                value={wealthConfig.commitment} 
                                onChange={handleCommitmentChange}
                                className="bg-transparent border-b border-rose-200 outline-none w-full text-right font-bold text-rose-700 focus:border-rose-500"
                            />
                        </div>
                    )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {/* Commitment Jar - Conditional Render */}
                {wealthConfig.showCommitment && (
                ...
                {wealthConfig.jars.map(jar => (
                    <div key={jar.id} className="bg-white border border-slate-100 p-6 rounded-3xl flex flex-col justify-between h-40 shadow-sm hover:shadow-md transition-shadow relative group">
                        <div className="flex justify-between items-start">
                            <div>
                                <div className="font-bold text-slate-700">{jar.label}</div>
                                <div className="text-xs bg-slate-100 px-2 py-0.5 rounded-full inline-block mt-1 font-bold text-slate-500">{jar.percent}%</div>
                            </div>
                ...
                {/* Add Jar Button */}
                <button onClick={() => setIsAddJarOpen(true)} className="border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center h-40 text-slate-400 hover:border-violet-400 hover:text-violet-500 transition-colors font-bold gap-2">
                    <Plus size={24}/> Add Jar
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-6 shadow-sm h-fit">
                    ...
                    <form onSubmit={addTransaction} className="space-y-4">
                        <input type="number" placeholder="Amount" value={expenseForm.amount} onChange={e=>setExpenseForm({...expenseForm, amount: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-violet-500"/>
                        <input type="date" value={expenseForm.date} onChange={e=>setExpenseForm({...expenseForm, date: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-violet-500"/>
                        <input type="text" placeholder="Category (e.g. Food)" value={expenseForm.category} onChange={e=>setExpenseForm({...expenseForm, category: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-violet-500"/>
                        <input type="text" placeholder="Remark" value={expenseForm.remark} onChange={e=>setExpenseForm({...expenseForm, remark: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-violet-500"/>
                        <button type="submit" className="w-full bg-slate-900 text-white font-bold py-3 rounded-xl hover:bg-slate-700 transition-all">Record Transaction</button>
                    </form>
                </div>
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <div className="text-xs font-bold text-slate-400 mt-1">
                                Net Balance: <span className={`text-base ${netTransactionTotal >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                    {netTransactionTotal >= 0 ? '+' : ''} RM {netTransactionTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </span>
                            </div>
                        </div>
                        <button onClick={() => setShowGraph(!showGraph)} className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors ${showGraph ? 'bg-violet-100 text-violet-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                            <PieChart size={14}/> {showGraph ? 'Show List' : 'GRAPH'}
                        </button>
                    </div>
                    ...
                    {showGraph ? (
                        <div className="flex flex-col items-center justify-center py-8">
                            ...
                            <div className="mt-8 grid grid-cols-2 gap-4 w-full">
                                {graphData.map(d => (
                                    <div key={d.name} className="flex items-center justify-between text-sm font-bold text-slate-600 bg-slate-50 p-2 rounded-lg">
                                        <div className="flex items-center gap-2">
                                            <div className="w-3 h-3 rounded-full" style={{backgroundColor: d.color}}></div>
                                            {d.name}
                                        </div>
                                        <span>RM {d.value}</span>
                                    </div>
                                ))}
                            </div>
                            <div className="mt-4 text-xl font-black text-slate-800">Total: RM {totalExpense.toLocaleString()}</div>
                        </div>
                    ) : (
                        <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                            {transactions.length === 0 && <div className="text-center text-slate-400 py-10">No transactions recorded.</div>}
                            {transactions.map(tx => (
                                <div key={tx.id} className="grid grid-cols-4 items-center p-3 hover:bg-slate-50 rounded-xl transition-colors border-b border-slate-50 text-sm">
                                    <div className="col-span-1 font-mono text-slate-400 text-xs">{tx.date}</div>
                                    <div className="col-span-2 font-bold text-slate-700 flex flex-col">
                                        <span>{tx.category}</span>
                                        <span className="text-[10px] text-slate-400 font-normal">{tx.remark}</span>
                                    </div>
                                    <div className={`col-span-1 text-right font-black ${tx.amount > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                ...
                <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                        ...
                        <form onSubmit={handleAddJar} className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-400 mb-1">Jar Name</label>
                                <input className="w-full border border-slate-200 rounded-xl p-2 outline-none focus:border-violet-500" value={newJarForm.label} onChange={e => setNewJarForm({...newJarForm, label: e.target.value})} autoFocus />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-400 mb-1">Percentage %</label>
                                <input type="number" className="w-full border border-slate-200 rounded-xl p-2 outline-none focus:border-violet-500" value={newJarForm.percent} onChange={e => setNewJarForm({...newJarForm, percent: e.target.value})} />
                            </div>
                            <div className="flex gap-2">
                                <button type="button" onClick={() => setIsAddJarOpen(false)} className="flex-1 py-2 text-slate-500 hover:bg-slate-50 rounded-xl">Cancel</button>
                                <button type="submit" className="flex-1 py-2 bg-slate-900 text-white rounded-xl font-bold">Add Jar</button>
                            </div>
                        </form>
                    </div>
                </div>
...
const CycleTrackerView = ({ data, setData, startYearDate, setStartYearDate }) => {
    // Strategy-like Progress bar calculation
    ...
    const handleDateChange = (e) => setStartYearDate(e.target.value);

    // Limit 5 tasks
    const addTask = (cycleId) => {
        setData(prev => prev.map(cycle => {
            if (cycle.id !== cycleId) return cycle;
            if ((cycle.tasks || []).length >= 5) {
                alert("Maximum 5 tasks allowed per 10-day cycle!");
                return cycle;
            }
    ...
    // Only allow typing new task if count < 5 or strict rules met (Simulated by UI constraint)
    const handleTaskKeyDown = (e, cycleId, tasks) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            addTask(cycleId);
        }
    ...
    const updateTask = (cycleId, taskId, field, val) => {
        setData(prev => prev.map(c => 
            c.id === cycleId ? { ...c, tasks: c.tasks.map(t => t.id === taskId ? { ...t, [field]: val } : t) } : c
        ));
    };

    const deleteTask = (cycleId, taskId) => {
        setData(prev => prev.map(c => 
            c.id === cycleId ? { ...c, tasks: c.tasks.filter(t => t.id !== taskId) } : c
        ));
    };
        ...
        <div className="h-full flex flex-col animate-fade-in pb-20">
            ...
            <div className="flex justify-between items-end mb-6 sticky top-0 bg-slate-50 z-20 py-2 border-b border-slate-200">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800 flex items-center gap-3">
                        <Activity className="text-violet-600" /> 36 x 10 Cycles
                    </h2>
                    <div className="flex items-center gap-4 mt-2">
                        <div className="bg-white border border-slate-200 px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-2">
                            ...
                            <input type="date" value={startYearDate} onChange={handleDateChange} className="bg-transparent text-slate-700 font-bold outline-none text-sm"/>
                        </div>
                    </div>
                </div>
                {/* Strategy Style Progress Bar */}
                <div className="w-1/3">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-6">
                ...
                <div className="grid grid-cols-12 gap-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <div className="col-span-4">Task List (Max 5)</div>
                    <div className="col-span-3">Action Plan</div>
                    <div className="col-span-3">Feedback</div>
                </div>

                {data.map(cycle => (
                    <div key={cycle.id} className="bg-white border border-slate-100 rounded-2xl p-6 grid grid-cols-12 gap-6 items-start shadow-sm hover:shadow-md transition-shadow">
                        {/* ID & Date - Aligned in one block */}
                        <div className="col-span-2 flex flex-col items-center justify-center h-full border-r border-slate-50">
                            <div className="w-12 h-12 bg-violet-50 text-violet-600 rounded-2xl flex items-center justify-center font-black text-xl mb-2 shadow-inner">
                                {cycle.id}
                            </div>
                            <div className="text-center text-[10px] font-bold text-slate-500 bg-slate-50 px-3 py-1 rounded-lg">
                                {cycle.dateRange}
                            </div>
                        </div>
                        ...
                        <div className="col-span-4 space-y-3 pt-2">
                            {(cycle.tasks || []).map(task => (
                                <div key={task.id} className="flex items-center gap-2 group">
                                    <button onClick={() => updateTask(cycle.id, task.id, 'done', !task.done)} className={`w-5 h-5 rounded border flex-shrink-0 flex items-center justify-center transition-colors ${task.done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300'}`}>
                                    ...
                                    <input 
                                        ...
                                        onKeyDown={e => handleTaskKeyDown(e, cycle.id, cycle.tasks)}
                                        placeholder="Task name..."
                                        className={`flex-1 bg-transparent border-b border-transparent focus:border-violet-400 outline-none text-sm py-1 ${task.done ? 'text-slate-400 line-through' : 'text-slate-700 font-medium'}`}
                                    />
                            ...
                            {(cycle.tasks || []).length === 0 && (
                                <button onClick={() => addTask(cycle.id)} className="text-xs font-bold text-violet-500 hover:underline flex items-center gap-1">+ Start Tasks</button>
                            )}
                        </div>

                        {/* Action Plans (Per Task) */}
                            ...
                            {(cycle.tasks || []).map(task => (
                                <div key={task.id} className="h-8 flex items-center"> 
                                    <input 
                                        ...
                                        className="w-full bg-slate-50 border border-transparent focus:border-violet-200 focus:bg-white rounded-lg px-3 py-1 text-xs outline-none transition-all"
                                    />
                                </div>
                            ))}
                        </div>
      ...
      <div className="h-full flex flex-col animate-fade-in pb-20 md:pb-0">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <span className="px-4 py-2 font-bold text-slate-700 text-sm flex items-center">{new Date(year, month).toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
        ...
        <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-100 border border-slate-100 flex-1 flex flex-col overflow-hidden">
          ...
          <div className="grid grid-cols-7 flex-1 auto-rows-fr bg-slate-50 gap-[1px]">
            {totalSlots.map((day, i) => {
              ...
              return (
                <div key={i} onClick={() => openAddModal(dateStr)} className="bg-white p-2 hover:bg-violet-50/30 transition-colors cursor-pointer group flex flex-col min-h-[100px] border-b border-r border-slate-50">
                  ...
                  <div className="space-y-1 overflow-hidden">
                    {dayTasks.slice(0, 3).map(t => (
                        <div key={t.id} className="text-[10px] px-2 py-1 rounded bg-slate-50 border border-slate-100 truncate text-slate-600">
                             {t.title}
                        </div>
                    ))}
                    {dayTasks.length > 3 && <div className="text-[9px] text-slate-400 pl-1 font-bold">+ {dayTasks.length - 3} more</div>}
                  </div>
                </div>
              );
            })}
...
const KanbanView = ({ currentDate, setCurrentDate, tasks, openAddModal, toggleTask, deleteTask, categoryColors }) => {
    // Vertical Timeline View: 6am - 11:59pm (Hours 6 to 23)
    ...
    return (
      <div className="h-full flex flex-col animate-fade-in pb-20 md:pb-0 bg-white/50 rounded-3xl border border-slate-100 shadow-sm">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-white rounded-t-3xl sticky top-0 z-10">
            <div>
               <h2 className="text-2xl font-black text-slate-800 tracking-tight">Daily Focus</h2>
               <p className="text-slate-500 text-sm font-bold">{currentDate.toLocaleDateString('default', {weekday: 'long', month: 'long', day: 'numeric'})}</p>
            </div>
            <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-xl">
               <button onClick={() => changeDay(-1)} className="p-2 hover:bg-white rounded-lg transition text-slate-400 shadow-sm"><ChevronLeft size={18}/></button>
               <button onClick={() => setCurrentDate(new Date())} className="text-xs font-bold px-3 py-1.5 bg-white text-violet-600 rounded-lg shadow-sm">Today</button>
               <button onClick={() => changeDay(1)} className="p-2 hover:bg-white rounded-lg transition text-slate-400 shadow-sm"><ChevronRight size={18}/></button>
            </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          <div className="space-y-2"> 
            {hours.map((hour) => {
              // Format: 6:00 am, 1:00 pm
              const ampm = hour >= 12 ? 'pm' : 'am';
              const hour12 = hour > 12 ? hour - 12 : hour;
              const displayHour = `${hour12}:00 ${ampm.toUpperCase()}`;
              // 24H Format for logic
              const timeLabel = `${hour.toString().padStart(2, '0')}:00`;
              
              // Filter tasks for this specific hour
              const hourTasks = tasks.filter(t => {
                  if (t.date !== dateStr) return false;
                  // If no time, default to 00:00 (which won't show here), or default to 6am if you prefer.
                  // Current logic: only show tasks that match this hour.
                          ...
                          <div className="space-y-2 w-full">
                              {hourTasks.map(task => (
                                  <TaskCard 
                                    ...
                                    showWarning={hourTasks.length > 1} // Warn if multiple tasks (user wants to focus on one)
                                  />
                              ))}
                          </div>
                      ) : (
                          <button 
                            onClick={() => openAddModal(dateStr, timeLabel)} // Pass exact time to modal
                            className="text-left text-slate-300 text-sm font-medium hover:text-violet-500 transition-colors flex items-center gap-2 py-2 w-full h-full"
                          >
                            <Plus size={16} className="opacity-50"/> Add focus
                          </button>
                      )}
                   </div>
                </div>
...
export default function App() {
  ...
  const [selectedTimeForAdd, setSelectedTimeForAdd] = useState(''); // NEW: State for pre-filled time
  ...
  const [wealthConfig, setWealthConfig] = useState({
      yearlyTarget: 100000,
      commitment: 2000, // Monthly fixed deduction default
      showCommitment: true,
      jars: [
          { id: 'savings', label: 'Savings 储蓄', percent: 50 },
          { id: 'investment', label: 'Investment 投资', percent: 50 },
      ]
  });
  ...
  useEffect(() => {
      if (user) {
          ...
          unsubs.push(onSnapshot(doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'cycles'), d => {
              if(d.exists()) {
                  setCyclesData(d.data().list || []);
                  setStartYearDate(d.data().startDate || new Date().getFullYear() + '-01-01');
              } else {
  ...
  const saveData = (type, data) => {
      if(user) {
          setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'data', type), data);
      } else {
          localStorage.setItem(`planner_${type}`, JSON.stringify(data));
      }
  };

  useEffect(() => { saveData('tasks', { list: tasks }); }, [tasks]);
  useEffect(() => { saveData('categories', { list: categories }); }, [categories]);
  useEffect(() => { saveData('cycles', { list: cyclesData, startDate: startYearDate }); }, [cyclesData, startYearDate]);
  useEffect(() => { saveData('wealth_v2', { balances: wealthBalances, transactions: wealthTransactions, config: wealthConfig }); }, [wealthBalances, wealthTransactions, wealthConfig]);
  ...
  const loadLocalStorage = () => {
      try {
          ...
          const cy = localStorage.getItem('planner_cycles'); 
          if(cy) {
              const d = JSON.parse(cy);
              setCyclesData(d.list || []);
              setStartYearDate(d.startDate);
          } else {
              setCyclesData(generateInitialCycles(new Date().getFullYear() + '-01-01'));
          }
          const w = localStorage.getItem('planner_wealth_v2');
          if(w) {
              const d = JSON.parse(w);
              setWealthBalances(d.balances || {});
              setWealthTransactions(d.transactions || []);
              if(d.config) setWealthConfig(d.config);
          }
  ...
  const generateInitialCycles = (startStr) => {
      ...
      for (let i = 0; i < 36; i++) {
        ...
        cycles.push({
          id: i + 1,
          dateRange: `${cycleStart.getMonth()+1}/${cycleStart.getDate()} - ${cycleEnd.getMonth()+1}/${cycleEnd.getDate()}`,
  ...
  // Updated openAddModal to accept timeStr
  const openAddModal = (dateStr, timeStr) => { 
      setSelectedDateForAdd(dateStr || getLocalDateString(new Date())); 
      setSelectedTimeForAdd(timeStr || ''); // Set the time
      setIsModalOpen(true); 
  };
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 font-sans text-slate-800 overflow-hidden selection:bg-violet-100 selection:text-violet-900">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-100 shadow-2xl md:shadow-none transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8">
          <div className="flex items-center gap-3 text-slate-900 font-black text-2xl mb-10 tracking-tight">
            <div className="w-10 h-10 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-violet-200"><Layout size={20} /></div>
          ...
          <nav className="space-y-1.5">
            {[
              ...
              { id: 'cycle', label: '36 x 10 Cycles', icon: Activity },
            ].map(item => (
              <button key={item.id} onClick={() => { setView(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3.5 px-5 py-3.5 rounded-2xl transition-all font-bold text-sm tracking-wide ${view === item.id ? 'bg-slate-900 text-white shadow-xl shadow-slate-200' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>
             ...
             <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center justify-between">
                {user ? (
                   <div className="flex items-center gap-3 overflow-hidden">
                       <div className="w-10 h-10 bg-violet-100 text-violet-600 rounded-full flex items-center justify-center font-bold">{user.email[0].toUpperCase()}</div>
                       <div className="flex-1 min-w-0">
                   ...
                   <button onClick={() => setIsAuthModalOpen(true)} className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white py-2.5 rounded-xl font-bold text-xs hover:bg-slate-800 transition-colors"><LogIn size={14} /> Log In / Sign Up</button>
                )}
             </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative h-full w-full overflow-hidden bg-slate-50">
        ...
        <div className="flex-1 p-5 md:p-10 overflow-y-auto custom-scrollbar md:pb-10 relative">
          {view === 'focus' && <DashboardView tasks={tasks} onAddTask={addTask} user={user} openAddModal={openAddModal} toggleTask={toggleTask} deleteTask={deleteTask} categoryColors={categoryColors} />}
          ...
          {view === 'kanban' && <KanbanView currentDate={currentDate} setCurrentDate={setCurrentDate} tasks={tasks} openAddModal={openAddModal} toggleTask={toggleTask} deleteTask={deleteTask} categoryColors={categoryColors} />}
