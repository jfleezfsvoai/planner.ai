...
const FlywheelView = ({ data, setData }) => {
    ...
    const addCard = (shape) => {
        const newCard = { id: generateId(), title: 'New Node', shape: shape || 'rounded', x: 50 + Math.random() * 100, y: 50 + Math.random() * 100, w: 140, h: 80, locked: false };
    ...
    const handleCardMouseDown = (e, id) => {
        ...
        if (!card || card.locked) return; // Prevent drag if locked
    ...
    const handleCardDoubleClick = (e, id) => {
        e.stopPropagation();
        setEditingCardId(id);
        setSelectedCardId(null); // Hide menu while editing
    };
    ...
    const renderConnection = (conn, isTemp = false) => {
        ...
        return (
            <g key={conn.id || 'temp'}>
                <path d={pathData} stroke="#94a3b8" strokeWidth="2" fill="none" markerEnd="url(#arrowhead)" />
                {!isTemp && (
    ...
    return (
        <div 
            className="w-full h-full relative overflow-hidden bg-slate-50 select-none animate-fade-in"
            onMouseMove={handleContainerMouseMove}
            onClick={handleContainerClick}
            ref={containerRef}
        >
            <div className="absolute top-4 left-4 z-20 bg-white p-2 rounded-xl shadow-lg border border-slate-100 flex gap-2 items-center">
                <span className="text-xs font-bold text-slate-400 px-2 uppercase tracking-wider">Shapes</span>
                <button onClick={(e) => { e.stopPropagation(); addCard('rounded'); }} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Rounded Square"><Square size={20} className="rounded-md"/></button>
                <button onClick={(e) => { e.stopPropagation(); addCard('rectangle'); }} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Rectangle"><Box size={20}/></button>
                <button onClick={(e) => { e.stopPropagation(); addCard('circle'); }} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Circle"><CircleIcon size={20}/></button>
                <button onClick={(e) => { e.stopPropagation(); addCard('triangle'); }} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Triangle"><Triangle size={20}/></button>
                <div className="w-[1px] h-6 bg-slate-200 mx-1"></div>
                <div className="px-2 text-xs text-slate-400">
                    {connectState ? <span className="text-violet-600 font-bold animate-pulse">Select target point...</span> : "Double-click to edit • Click to open options"}
                </div>
            </div>
            ...
            {cards.map(card => {
                ...
                if (card.shape === 'circle') shapeClass = "rounded-full flex items-center justify-center aspect-square";
                if (card.shape === 'triangle') shapeClass = "clip-triangle aspect-square flex items-center justify-center pt-8"; 
                ...
                return (
                    <div
                        ...
                        style={{ 
                            ...
                            top: card.y, 
                            width: card.w,
                            height: isCircleOrTriangle ? card.w : card.h,
                            clipPath: card.shape === 'triangle' ? 'polygon(50% 0%, 0% 100%, 100% 100%)' : 'none',
                            backgroundColor: 'white'
                        }}
                        className={`absolute z-10 shadow-md border group transition-shadow flex flex-col items-center justify-center
                            ${shapeClass} 
                            ${card.locked ? 'border-slate-200 cursor-default' : 'border-slate-300 cursor-move hover:ring-2 hover:ring-violet-200'}
                            ${isSelected ? 'ring-2 ring-violet-400 border-transparent' : ''}
                        `}
                        onMouseDown={(e) => handleCardMouseDown(e, card.id)}
                        onClick={(e) => handleCardClick(e, card.id)}
                        onDoubleClick={(e) => handleCardDoubleClick(e, card.id)}
                    >
                        {card.shape === 'triangle' && <div className="absolute inset-0 bg-white z-[-1]"></div>}

                        {/* Option Toolbar (shows when clicked, hides when dragging or editing) */}
                        {isSelected && !isEditing && (
                            <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-slate-800 text-white p-1.5 rounded-xl shadow-xl z-50 cursor-default" onMouseDown={e => e.stopPropagation()}>
                        ...
                        {/* Lock Icon Indicator */}
                        {card.locked && !isSelected && <Lock size={10} className="absolute top-3 right-3 text-slate-300 pointer-events-none" />}

                        {/* Content Area */}
                        <div className="relative w-full h-full flex flex-col p-2 items-center justify-center z-10">
                            <textarea 
                                ...
                                readOnly={!isEditing}
                                className={`w-full bg-transparent resize-none outline-none font-bold text-slate-700 text-sm text-center overflow-hidden 
                                    ${isEditing ? 'pointer-events-auto bg-slate-50 p-1 rounded border border-violet-200' : 'pointer-events-none'}`}
                                placeholder="Text..."
                                rows={3}
                        ...
                        {['top', 'right', 'bottom', 'left'].map(pos => {
                            ...
                            if (pos === 'right') style = { right: -5, top: '50%', transform: 'translateY(-50%)' };
                            if (pos === 'bottom') style = { bottom: -5, left: '50%', transform: 'translateX(-50%)' };
                            if (pos === 'left') style = { left: -5, top: '50%', transform: 'translateY(-50%)' };

                            return (
                                <div
                                    key={pos}
                                    className={`absolute w-3.5 h-3.5 bg-white border-2 border-violet-400 rounded-full cursor-pointer hover:bg-violet-600 hover:scale-125 transition-all z-20 
  ...
  if (isEditing) {
      return (
        <div className="bg-white p-3 rounded-2xl border border-violet-200 shadow-md mb-2 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="mb-3"><div className="flex gap-2 items-center mb-2">{isCustomCategory ? (<div className="flex-1 flex flex-col gap-2"><div className="relative"><input type="text" value={editCategory} onChange={e => setEditCategory(e.target.value)} placeholder="类别名称" className="w-full bg-slate-50 border border-slate-200 rounded p-1.5 pr-6 text-xs outline-none focus:border-violet-500" /><button type="button" onClick={() => setIsCustomCategory(false)} className="absolute right-1 top-1/2 -translate-y-1/2 text-slate-400"><X size={12}/></button></div><div className="flex gap-1 flex-wrap">{CATEGORY_COLORS.map((col) => (<button key={col.id} type="button" onClick={() => setCustomCategoryColor(col.value)} className={`w-3 h-3 rounded-full border ${col.value.split(' ')[0]} ${customCategoryColor === col.value ? 'ring-1 ring-offset-1 ring-slate-400' : ''}`} />))}</div></div>) : (<><select value={editCategory} onChange={(e) => setEditCategory(e.target.value)} className="flex-1 text-xs bg-slate-50 border border-slate-200 rounded p-1.5 outline-none">{(categories || []).map(c => <option key={c.name} value={c.name}>{c.name}</option>)}</select><button type="button" onClick={() => { setIsCustomCategory(true); setEditCategory(safeCategory); }} className="p-1.5 bg-slate-100 hover:bg-violet-100 text-violet-600 rounded" title="Edit/New"><Edit3 size={12}/></button></>)}</div></div>
            <div className="mb-3"><label className="block text-[10px] font-bold text-slate-400 mb-1">优先级</label><div className="grid grid-cols-2 gap-1">{Object.entries(PRIORITIES).map(([key, config]) => (<button key={key} type="button" onClick={() => togglePriority(key)} className={`text-[9px] px-1 py-1 rounded border transition-all truncate ${editPriority === key ? config.color + ' ring-1 ring-slate-200' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>{config.label}</button>))}</div></div>
            <div className="flex justify-end gap-2"><button onClick={() => setIsEditing(false)} className="p-1.5 text-slate-400 hover:bg-slate-50 rounded-lg"><X size={14}/></button><button onClick={handleSave} className="p-1.5 bg-violet-500 text-white rounded-lg hover:bg-violet-600"><Save size={14}/></button></div>
        </div>
      );
  }
  return (
    <div ref={cardRef} draggable onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={`bg-white/80 backdrop-blur-sm p-3 rounded-2xl border transition-all group relative mb-2 cursor-grab active:cursor-grabbing ${showWarning ? 'border-amber-300 shadow-amber-100' : 'border-slate-100 shadow-sm hover:border-violet-200'} ${highlightStyle}`}>
        {dropPosition === 'top' && (<div className="absolute -top-1 left-0 right-0 h-1 bg-violet-500 rounded-full z-10 pointer-events-none animate-in fade-in duration-150"></div>)}
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0" onDoubleClick={() => setIsEditing(true)}>
          <div className="flex justify-between items-start">
             <div className="flex flex-wrap items-center gap-2"><p className={`text-xs font-bold truncate transition-colors ${task.completed ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{task.title}</p>{format === 'timeline' && <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider border ${categoryStyle}`}>{safeCategory}</span>}</div>
        ...
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all bg-white/80 rounded-lg backdrop-blur-sm"><button onClick={(e) => { e.stopPropagation(); setIsEditing(true); }} className="text-slate-400 hover:text-violet-500 p-1" title="编辑"><Edit3 size={12} /></button><button onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} className="text-slate-400 hover:text-red-500 p-1" title="删除"><Trash2 size={12} /></button></div>
      </div>
      {dropPosition === 'bottom' && (<div className="absolute -bottom-1 left-0 right-0 h-1 bg-violet-500 rounded-full z-10 pointer-events-none animate-in fade-in duration-150"></div>)}
    </div>
  );
};
...
const HabitTracker = ({ habits, onUpdate, onAdd, onDelete }) => {
    ...
    return (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 overflow-hidden">
            <div className="flex justify-between items-center mb-6"><h3 className="font-bold text-slate-800 flex items-center gap-2"><CheckCircle2 className="text-emerald-500"/> Habit Tracker ({today.toLocaleString('default', { month: 'long' })})</h3></div>
            <div className="overflow-x-auto custom-scrollbar pb-4"><table className="w-full min-w-[800px] border-collapse"><thead><tr className="border-b border-slate-100"><th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider pb-4 px-2 min-w-[150px] sticky left-0 bg-white z-20">习惯养成</th><th className="text-center text-xs font-black text-slate-400 uppercase tracking-wider pb-4 px-2 min-w-[80px]">目标</th><th className="text-center text-xs font-black text-slate-400 uppercase tracking-wider pb-4 px-2 min-w-[60px]">完成</th><th className="text-center text-xs font-black text-slate-400 uppercase tracking-wider pb-4 px-2 min-w-[100px]">进度</th>{daysInMonth.map((d, i) => (<th key={i} className="text-center pb-4 px-1 min-w-[40px]"><div className={`flex flex-col items-center ${getLocalDateString(d) === getLocalDateString(today) ? 'text-violet-600 bg-violet-50 rounded-lg p-1' : 'text-slate-400'}`}><span className="text-[9px] font-bold uppercase">{d.toLocaleDateString('default', {weekday: 'narrow'})}</span><span className="text-xs font-bold">{d.getDate()}</span></div></th>))}<th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider pb-4 px-4 min-w-[150px]">完成奖励</th><th className="w-10 pb-4"></th></tr></thead><tbody className="text-sm">{habits.map(habit => { const completedCount = getCurrentMonthCompletedCount(habit.completed); const target = habit.target || daysInMonth.length; const progress = Math.min(100, Math.round((completedCount / target) * 100)); return ( <tr key={habit.id} className="group hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0"><td className="py-3 px-2 font-bold text-slate-700 sticky left-0 bg-white group-hover:bg-slate-50 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">{habit.name}</td><td className="py-3 px-2 text-center"><input type="number" value={habit.target || daysInMonth.length} onChange={(e) => handleUpdateField(habit.id, 'target', parseInt(e.target.value))} className="w-12 text-center bg-transparent border-b border-dashed border-slate-300 focus:border-violet-500 outline-none text-slate-600 font-medium"/></td><td className="py-3 px-2 text-center font-bold text-emerald-600">{completedCount}</td><td className="py-3 px-2 text-center"><div className="flex items-center gap-2 justify-center"><div className="w-12 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-400 rounded-full transition-all duration-500" style={{width: `${progress}%`}}></div></div><span className="text-[10px] text-slate-400 font-bold">{progress}%</span></div></td>{daysInMonth.map((d, i) => { const dateStr = getLocalDateString(d); const isDone = habit.completed?.includes(dateStr); return (<td key={i} className="text-center py-3 px-1"><button onClick={() => toggleHabit(habit.id, dateStr)} className={`w-5 h-5 rounded flex items-center justify-center transition-all mx-auto ${isDone ? 'bg-emerald-500 text-white shadow-sm' : 'bg-white border border-slate-200 text-transparent hover:border-emerald-300'}`}><CheckSquare size={12} fill={isDone ? "currentColor" : "none"}/></button></td>); })}<td className="py-3 px-4"><div className="flex items-center gap-2"><Gift size={14} className={habit.reward ? "text-rose-400" : "text-slate-300"}/><input type="text" value={habit.reward || ''} onChange={(e) => handleUpdateField(habit.id, 'reward', e.target.value)} placeholder="Set reward..." className="w-full bg-transparent border-b border-transparent focus:border-slate-300 outline-none text-xs text-slate-600 placeholder-slate-300"/></div></td><td className="text-right py-3 px-2"><button onClick={() => onDelete(habit.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16}/></button></td></tr> ); })}</tbody></table></div>
            <form onSubmit={handleAdd} className="mt-6 flex gap-3 border-t border-slate-50 pt-4"><input type="text" value={newHabit} onChange={e => setNewHabit(e.target.value)} placeholder="New habit name..." className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none text-sm focus:border-emerald-500 transition-all"/><input type="number" value={newHabitTarget} onChange={e => setNewHabitTarget(parseInt(e.target.value))} placeholder="Target days" className="w-24 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none text-sm focus:border-emerald-500 transition-all text-center"/><button type="submit" className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg flex items-center gap-2"><Plus size={16}/> Add Habit</button></form>
        </div>
    );
};
    ...
    <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-md z-[70] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden border border-white/50">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div><label className="block text-xs font-bold text-slate-400 mb-1.5 uppercase tracking-wider">内容</label><input ref={inputRef} type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:border-violet-500 outline-none" placeholder="需要做什么？"/></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs font-bold text-slate-400 mb-1.5 uppercase tracking-wider">类别</label><div className="flex gap-2">{isCustomCategory ? (<div className="flex-1 flex flex-col gap-2"><div className="relative"><input type="text" value={category} onChange={e => setCategory(e.target.value)} placeholder="新类别名称" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 pr-8 text-sm outline-none focus:border-violet-500" autoFocus/><button type="button" onClick={() => setIsCustomCategory(false)} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400"><X size={14}/></button></div><div className="flex gap-1 flex-wrap">{CATEGORY_COLORS.map((col) => (<button key={col.id} type="button" onClick={() => setCustomCategoryColor(col.value)} className={`w-4 h-4 rounded-full border ${col.value.split(' ')[0]} ${customCategoryColor === col.value ? 'ring-2 ring-offset-1 ring-slate-400' : ''}`}/>))}</div></div>) : (<div className="flex gap-2 w-full items-start"><select value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm outline-none focus:border-violet-500 appearance-none">{categories.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}</select><button type="button" onClick={() => { setIsCustomCategory(true); setCategory(''); }} className="p-3 bg-slate-100 hover:bg-violet-100 text-violet-600 rounded-xl"><Plus size={18}/></button></div>)}</div></div>
          ...
          <div><label className="block text-xs font-bold text-slate-400 mb-1.5 uppercase tracking-wider">优先级 (可选)</label><div className="grid grid-cols-2 gap-2">{Object.entries(PRIORITIES).map(([key, config]) => (<button key={key} type="button" onClick={() => togglePriority(key)} className={`text-xs p-2 rounded-lg border transition-all text-center ${priority === key ? config.color + ' ring-2 ring-offset-1 ring-slate-200' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>{config.label}</button>))}</div></div>
...
const AuthModal = ({ isOpen, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[60] flex items-center justify-center p-4 animate-in fade-in"><div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden p-8 border border-white/50 relative"><button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600"><X size={20}/></button><div className="text-center mb-6"><div className="w-12 h-12 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-2xl mx-auto flex items-center justify-center text-white mb-4 shadow-lg shadow-violet-200"><User size={24} /></div><h2 className="text-2xl font-black text-slate-800">{isLogin ? 'Welcome Back' : 'Create Account'}</h2><p className="text-slate-500 text-sm mt-1">Sync your data across devices</p></div>{error && <div className="bg-red-50 text-red-500 text-xs p-3 rounded-xl mb-4 text-center font-bold">{error}</div>}<form onSubmit={handleAuth} className="space-y-4"><input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm outline-none focus:border-violet-500" required /><input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm outline-none focus:border-violet-500" required /><button type="submit" disabled={loading} className="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-slate-800 transition-all disabled:opacity-50">{loading ? 'Processing...' : (isLogin ? 'Log In' : 'Sign Up')}</button></form><div className="mt-6 text-center text-xs text-slate-500 font-medium">{isLogin ? "No account? " : "Have account? "}<button onClick={() => setIsLogin(!isLogin)} className="text-violet-600 font-bold hover:underline">{isLogin ? 'Sign Up' : 'Log In'}</button></div></div></div>
    );
};
...
const DayPreviewModal = ({ isOpen, onClose, dateStr, tasks, onToggle, onUpdate, onDelete, categories, setCategories }) => {
    ...
    const startEdit = (task) => { setEditingId(task.id); setEditTitle(task.title); setEditCategory(task.category); };
    const saveEdit = () => { if (editTitle.trim()) { onUpdate(editingId, { title: editTitle, category: editCategory }); } setEditingId(null); };
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[80vh]">
                <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center"><div><h3 className="font-bold text-lg text-slate-800">任务概览</h3><p className="text-xs font-bold text-violet-500 uppercase tracking-widest">{dateStr}</p></div><button onClick={onClose} className="p-2 bg-white rounded-full text-slate-400 hover:text-slate-600 shadow-sm"><X size={20}/></button></div>
                <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                    {dayTasks.length === 0 ? <div className="text-center py-12 text-slate-400 italic">本日暂无安排</div> : dayTasks.map(task => (
                        <div key={task.id} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 group">
                            <button onClick={() => onToggle(task.id)} className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-all flex-shrink-0 ${task.completed ? 'bg-violet-500 border-violet-500 text-white' : 'bg-white border-slate-300'}`}><CheckSquare size={12} fill={task.completed ? "currentColor" : "none"}/></button>
                            {editingId === task.id ? (
                                <div className="flex-1 space-y-2"><input value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full bg-white border border-slate-200 rounded p-1.5 text-sm outline-none focus:border-violet-500" autoFocus/><div className="flex gap-2"><select value={editCategory} onChange={e => setEditCategory(e.target.value)} className="flex-1 text-xs bg-white border border-slate-200 rounded p-1 outline-none">{(categories || ['工作', '生活', '健康', '学习']).map(c => <option key={c.name} value={c.name}>{c.name}</option>)}</select><button onClick={saveEdit} className="p-1.5 bg-violet-500 text-white rounded hover:bg-violet-600"><Save size={12}/></button></div></div>
                            ) : (
                                <div className="flex-1 min-w-0"><div className="flex justify-between"><span className={`text-sm font-medium break-words ${task.completed ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{task.title}</span></div><div className="flex items-center gap-2 mt-1"><span className="text-[10px] px-1.5 py-0.5 rounded bg-white border border-slate-100 text-slate-500">{task.category}</span>{task.time && <span className="text-xs text-slate-400 font-mono">{task.time}</span>}</div></div>
                            )}
                            {editingId !== task.id && (<div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => startEdit(task)} className="p-1 text-slate-400 hover:text-violet-500"><Edit3 size={14}/></button><button onClick={() => onDelete(task.id)} className="p-1 text-slate-400 hover:text-red-500"><Trash2 size={14}/></button></div>)}
                        </div>
                    ))}
                </div>
            </div>
...
const DashboardView = ({ tasks, onAddTask, user, openAddModal, toggleTask, deleteTask, onUpdate, moveTask, categoryColors, categories, habits, onUpdateHabit, onAddHabit, onDeleteHabit, setCategories }) => {
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-24">
        <header><h2 className="text-3xl font-black text-slate-800">Dashboard</h2><p className="text-slate-500 font-medium">Welcome back, <span className="text-violet-600">{user?.email?.split('@')[0] || 'Commander'}</span></p></header>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className={`md:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col overflow-hidden ${containerHeight}`}>
                <div className="flex justify-between items-center p-6 pb-2 shrink-0"><h3 className="font-bold text-slate-800 flex items-center gap-2"><Target className="text-rose-500"/> Today's Focus</h3><button onClick={() => openAddModal(todayStr)} className="bg-slate-900 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-700"><Plus size={16}/></button></div>
                <div className="flex-1 overflow-y-auto p-6 pt-2 custom-scrollbar space-y-2">{todaysTasks.length === 0 ? <div className="text-center text-slate-400 py-10">No tasks for today.</div> : todaysTasks.map(task => (<TaskCard key={task.id} task={task} onToggle={toggleTask} onDelete={deleteTask} onUpdate={onUpdate} moveTask={moveTask} categoryColors={categoryColors} categories={categories} setCategories={setCategories}/>))}</div>
            </div>
            <div className={`bg-white rounded-3xl border border-slate-100 shadow-sm p-6 overflow-y-auto custom-scrollbar ${containerHeight}`}><h3 className="font-bold text-slate-800 flex items-center gap-2 mb-6"><PieChart className="text-blue-500"/> Analysis</h3><div className="space-y-4">{Object.entries(catStats).map(([cat, stat]) => (<div key={cat}><div className="flex justify-between text-xs font-bold text-slate-600 mb-1"><span>{cat}</span><span>{stat.completed}/{stat.total}</span></div><div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-blue-500 h-2 rounded-full transition-all" style={{width: `${(stat.completed/stat.total)*100}%`}}></div></div></div>))}</div></div>
        </div>
...
const WealthJarView = ({ balances, setBalances, wealthConfig, setWealthConfig, transactions = [], setTransactions }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-24">
             <div className="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl"><div className="relative z-10 flex justify-between items-end"><div><div className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">年度储蓄目标 (储蓄+投资)</div><div className="text-4xl font-black">RM {savingsPlusInvestment.toLocaleString()} <span className="text-slate-500 text-2xl font-bold"> / {wealthConfig.yearlyTarget.toLocaleString()}</span></div></div><button onClick={() => { const n = prompt("New Target:", wealthConfig.yearlyTarget); if(n) setWealthConfig({...wealthConfig, yearlyTarget: parseFloat(n)}); }} className="bg-white/10 px-4 py-2 rounded-xl text-sm font-bold transition-all">Edit</button></div><div className="mt-6 w-full bg-white/10 rounded-full h-2"><div className="bg-emerald-400 h-2 rounded-full transition-all duration-700" style={{ width: `${Math.min(100, (savingsPlusInvestment / wealthConfig.yearlyTarget) * 100)}%` }}></div></div></div>
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm"><h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><DollarSign className="text-emerald-500"/> 收入分配器</h3><form onSubmit={handleDistribute} className="flex flex-col md:flex-row gap-4"><input type="number" placeholder="输入收入 (RM)" value={income} onChange={e=>setIncome(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-emerald-500 font-bold text-lg"/>{wealthConfig.showCommitment && (<div className="flex items-center gap-2 px-4 py-3 bg-rose-50 rounded-xl border border-rose-100 text-rose-600 font-bold min-w-[200px]"><span className="text-xs uppercase whitespace-nowrap">固定开销:</span> <input type="number" value={wealthConfig.commitment} onChange={e => setWealthConfig({...wealthConfig, commitment: parseFloat(e.target.value)||0})} className="bg-transparent border-b border-rose-200 outline-none w-full text-right font-bold" /></div>)}<button type="submit" className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-700 transition-all">全部分配</button></form></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{wealthConfig.showCommitment ? (<div className="bg-rose-50 border border-rose-100 p-6 rounded-3xl flex flex-col justify-between h-40 relative group"><div className="flex justify-between items-start"><div className="font-bold text-rose-700">固定开销</div><Lock size={16} className="text-rose-400"/></div><div className="text-2xl font-black text-rose-800">RM {(balances.commitment||0).toLocaleString()}</div></div>) : (<button onClick={restoreCommitment} className="bg-rose-50/50 border-2 border-dashed border-rose-200 p-6 rounded-3xl flex flex-col items-center justify-center h-40 text-rose-400 hover:bg-rose-50 hover:text-rose-600 transition-all font-bold gap-2"><RefreshCw size={24}/> 恢复固定开销</button>)}{wealthConfig.jars.map(jar => (<div key={jar.id} className="bg-white border border-slate-100 p-6 rounded-3xl flex flex-col justify-between h-40 shadow-sm hover:shadow-md transition-all relative group"><div className="flex justify-between items-start"><div><div className="font-bold text-slate-700">{jar.label}</div><div className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-full inline-block mt-1 font-bold">{jar.percent}%</div></div><div className="p-2 bg-slate-50 rounded-full text-slate-400">{getIconForLabel(jar.label)}</div></div><div className="text-2xl font-black text-slate-800">RM {(balances[jar.id]||0).toLocaleString()}</div><button onClick={() => deleteJar(jar.id)} className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 text-red-400 hover:bg-red-50 p-1 rounded transition-all"><X size={16}/></button></div>))}<button onClick={() => setIsAddJarOpen(true)} className="border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center h-40 text-slate-400 hover:border-violet-400 font-bold gap-2 transition-all"><Plus size={24}/> 添加存钱罐</button></div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-6 shadow-sm h-fit"><h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">{editingTxId ? <Edit3 size={18} className="text-amber-500"/> : <DollarSign size={18} className="text-slate-400"/>}{editingTxId ? '编辑交易记录' : '记录收支'}</h3><form onSubmit={submitTransaction} className="space-y-4"><input type="number" placeholder="金额" value={expenseForm.amount} onChange={e=>setExpenseForm({...expenseForm, amount: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:ring-2 ring-violet-100" /><div className="flex gap-2">{!isCustomCat ? (<select value={expenseForm.category} onChange={e => e.target.value === 'NEW' ? setIsCustomCat(true) : setExpenseForm({...expenseForm, category: e.target.value})} className="flex-1 p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none"><option value="">选择类别</option>{usedTxCats.map(c => <option key={c} value={c}>{c}</option>)}<option value="NEW" className="font-bold text-violet-600">+ 新增类别</option></select>) : (<div className="flex-1 relative"><input type="text" placeholder="新类别名称" value={expenseForm.category} onChange={e => setExpenseForm({...expenseForm, category: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none" autoFocus/><button type="button" onClick={() => setIsCustomCat(false)} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400"><X size={14}/></button></div>)}</div><input type="text" placeholder="备注" value={expenseForm.remark} onChange={e=>setExpenseForm({...expenseForm, remark: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none" /><input type="date" value={expenseForm.date} onChange={e=>setExpenseForm({...expenseForm, date: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none" /><div className="flex gap-2">{editingTxId && (<button type="button" onClick={() => {setEditingTxId(null); setExpenseForm({ amount: '', category: '', remark: '', date: getLocalDateString(new Date()) });}} className="flex-1 bg-slate-100 text-slate-600 font-bold py-3 rounded-xl hover:bg-slate-200 transition-all">取消</button>)}<button type="submit" className={`flex-[2] text-white font-bold py-3 rounded-xl transition-all shadow-lg ${editingTxId ? 'bg-amber-500 hover:bg-amber-600' : 'bg-slate-900 hover:bg-slate-800'}`}>{editingTxId ? '保存修改' : '录入数据'}</button></div></form></div>
                <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 p-6 shadow-sm min-h-[400px]"><div className="flex justify-between items-center mb-6"><div><h3 className="font-bold text-slate-800">近期流水</h3><div className="text-xs font-bold text-slate-400 mt-1">余额变动: <span className={netTransactionTotal >= 0 ? 'text-emerald-500' : 'text-rose-500'}>{netTransactionTotal >= 0 ? '+' : ''} RM {netTransactionTotal.toLocaleString()}</span></div></div><button onClick={() => setShowGraph(!showGraph)} className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${showGraph ? 'bg-violet-600 text-white shadow-lg shadow-violet-100' : 'bg-slate-100 text-slate-600'}`}>{showGraph ? <Layout size={14}/> : <BarChart2 size={14}/>} {showGraph ? '返回列表' : '分类统计'}</button></div>{showGraph ? (<div className="py-4"><h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">各类别净额对比 (横向)</h4><HorizontalBarChart data={barData} /></div>) : (<div className="space-y-6 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">{Object.keys(groupedTransactions).length === 0 ? <div className="text-center text-slate-400 py-10 italic">暂无收支记录。</div> : Object.entries(groupedTransactions).map(([date, txs]) => (<div key={date} className="space-y-2"><div className="sticky top-0 bg-white/95 backdrop-blur-sm z-10 py-1 border-b border-slate-50"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{date === getLocalDateString(new Date()) ? '今天' : date}</span></div><div className="space-y-1">{txs.map(tx => (<div key={tx.id} className="grid grid-cols-12 items-center p-3 hover:bg-slate-50 rounded-2xl transition-all border border-transparent hover:border-slate-100 group"><div className="col-span-7"><div className="font-bold text-slate-700 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-slate-200"></span>{tx.category}</div><div className="text-[10px] text-slate-400 font-medium pl-3.5">{tx.remark || '无备注'}</div></div><div className={`col-span-3 text-right font-black text-sm ${tx.amount > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{tx.amount > 0 ? '+' : ''} RM {Math.abs(tx.amount).toFixed(2)}</div><div className="col-span-2 flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all"><button onClick={() => startEditTx(tx)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="编辑"><Edit3 size={14}/></button><button onClick={() => deleteTx(tx.id)} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="删除"><Trash2 size={14}/></button></div></div>))}</div></div>))}</div>)}</div>
            </div>
            {isAddJarOpen && (<div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4"><div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl border border-white/50"><h3 className="font-bold text-xl mb-6 text-slate-800">添加新存钱罐</h3><form onSubmit={handleAddJar} className="space-y-5"><div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">罐子名称</label><input className="w-full border border-slate-200 rounded-xl p-3 outline-none focus:border-violet-500 bg-slate-50" value={newJarForm.label} onChange={e => setNewJarForm({...newJarForm, label: e.target.value})} placeholder="例如: 长期储蓄" autoFocus /></div><div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">分配比例 (%)</label><input type="number" className="w-full border border-slate-200 rounded-xl p-3 outline-none focus:border-violet-500 bg-slate-50" value={newJarForm.percent} onChange={e => setNewJarForm({...newJarForm, percent: e.target.value})} placeholder="0 - 100" /></div><div className="flex gap-3 pt-2"><button type="button" onClick={() => setIsAddJarOpen(false)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-xl transition-all">取消</button><button type="submit" className="flex-1 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg">创建罐子</button></div></form></div></div>)}
        </div>
    );
};
            ...
            <div className="flex items-center gap-4">
                <div className="flex flex-row gap-3 items-center whitespace-nowrap">
                   ...
                   <button 
                       ...
                       className="flex items-center gap-1 px-4 py-2 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-violet-100 hover:text-violet-600 transition-all text-xs"
                ...
                <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl">
                    ...
                    <input 
                        ...
                        className="bg-transparent font-bold text-xs px-1 outline-none text-slate-600 min-w-[140px] text-center cursor-pointer w-auto"
                    />
    ...
    return (
        <div className={`p-5 rounded-3xl border bg-white border-slate-100 shadow-sm`}>
            ...
            {Array.from({ length: count }).map((_, i) => {
                ...
                return (<ReviewInput key={i} value={data[i]} onChange={(val) => onChange(field, i, val)} placeholder={`Item ${i+1}`} color={color} showCheckbox={showCheckbox} disabled={!isVisible} />);
            })}
        </div>
    );
};
...
const ReviewView = ({ reviews, onUpdateReview, startYearDate }) => {
    ...
    const handleCycleChange = (newBlocks) => {
        const newData = { ...reviews, cycle: { ...(reviews.cycle || {}), [activeGlobalCycleId]: { ...cycleData, blocks: newBlocks } } };
        onUpdateReview(newData);
    };
    ...
    const handleAddBlock = () => {
        const blocks = cycleData.blocks || [{ plan: '', execute: '', adjust: '', check: '' }];
        if (blocks.length < 5) {
            handleCycleChange([...blocks, { plan: '', execute: '', adjust: '', check: '' }]);
        }
    };
    ...
    const handleDeleteBlock = (index) => {
        ...
        // If empty, keep at least one
        handleCycleChange(newBlocks.length === 0 ? [{ plan: '', execute: '', adjust: '', check: '' }] : newBlocks);
    };
    ...
    return (
        <div className="max-w-6xl mx-auto pb-24 space-y-8 animate-fade-in">
            <header className="flex justify-between items-end"><div><h2 className="text-3xl font-black text-slate-800">Review</h2><p className="text-slate-500 font-medium">Reflect and Evolve</p></div><div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm">{['daily', 'cycle', 'yearly'].map(t => (<button key={t} onClick={() => setActiveTab(t)} className={`px-6 py-2 rounded-xl text-sm font-bold transition-all capitalize ${activeTab === t ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{t} Review</button>))}</div></header>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="space-y-4">
                            {(cycleData.blocks || [{ plan: '', execute: '', adjust: '', check: '' }]).map((block, i) => (
                                <div key={i} className="p-6 rounded-3xl border bg-white border-slate-100 shadow-sm relative animate-fade-in">
                                    <div className="flex justify-between items-center mb-4">
                                        <h4 className="font-black text-slate-400 uppercase tracking-wider text-xs flex items-center gap-2">
                                            <Target size={14} className="text-violet-500"/> Block {i + 1}
                                        </h4>
                                        <button onClick={() => handleDeleteBlock(i)} className="text-slate-300 hover:text-red-500 transition-colors p-1" title="Delete Block">
                                            <Trash2 size={14}/>
                                        </button>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-blue-500 uppercase tracking-widest ml-1">Plan (计划)</label>
                                            <ReviewInput value={block.plan} onChange={(v) => handleBlockChange(i, 'plan', v)} placeholder="What's the plan?" color="blue" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-violet-500 uppercase tracking-widest ml-1">Execute (执行)</label>
                                            <ReviewInput value={block.execute} onChange={(v) => handleBlockChange(i, 'execute', v)} placeholder="How did it go?" color="violet" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-amber-500 uppercase tracking-widest ml-1">Adjust (调整)</label>
                                            <ReviewInput value={block.adjust} onChange={(v) => handleBlockChange(i, 'adjust', v)} placeholder="What to change?" color="amber" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest ml-1">Check (检查)</label>
                        ...
                        {(!cycleData.blocks || cycleData.blocks.length < 5) && (
                            <button onClick={handleAddBlock} className="w-full py-4 rounded-3xl border-2 border-dashed border-slate-200 text-slate-400 font-bold hover:border-violet-300 hover:text-violet-500 hover:bg-violet-50 transition-all flex items-center justify-center gap-2">
...
export default function App() {
  ...
  const moveTask = (dragId, targetId, position) => {
      ...
      if (dragIndex >= 0 && targetIndex >= 0 && dragIndex !== targetIndex) {
          ...
          if (position === 'bottom') { insertIndex = newTargetIndex + 1; } 
          else { insertIndex = newTargetIndex; }
  ...
  const cloneYesterdayTasks = (targetDateStr, sourceDateStr) => {
      ...
      const clonedTasks = tasksToClone.map(t => ({
          ...t, id: generateId(), date: targetDateStr, completed: false
      }));
