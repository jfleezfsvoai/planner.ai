 ...
 * Refactored: Components moved outside App to prevent focus loss re-renders.

 */

// --- 🔴 用这一大段替换掉原来的 firebaseConfig 配置区域 ---
    ...
    <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-violet-200 transition-all group relative mb-3">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p
            className={`text-sm font-semibold tracking-wide truncate transition-colors ${
          ...
          <div className="flex items-center gap-2 mt-2">
            <span
              className={`text-[10px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider ${getCategoryStyle(
                task.category
              )}`}
            >
              {task.category}
    ...
    <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden scale-100 transition-transform border border-white/50">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-violet-200 active:scale-[0.98]"
          >
            确认添加
          </button>
        </form>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden p-8 border border-white/50 relative">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Sync your planner data across devices
          </p>
        </div>

        {error && (
          <div className="bg-red-50 text-red-500 text-xs p-3 rounded-xl mb-4 text-center font-bold">
            {error}
          </div>
        )}
...
const WealthJarView = ({ balances, setBalances }) => {
  ...
  const handleDistribute = (e) => {
    ...
    JARS_CONFIG.forEach((jar) => {
      newBalances[jar.id] += amount * (jar.percent / 100);
    });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-24">
      <div className="flex justify-between items-end">
        ...
        <button
          onClick={handleReset}
          className="text-xs text-slate-400 hover:text-red-500 transition-colors font-medium px-3 py-1 rounded-full hover:bg-slate-100"
        >
          重置系统
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-xl shadow-slate-100 border border-slate-100 relative overflow-hidden">
        ...
        <form
          ...
          className="flex flex-col md:flex-row gap-6 items-end relative z-10"
        >
          ...
          <button
            ...
            className="w-full md:w-auto px-10 py-5 rounded-2xl bg-slate-900 text-white font-bold text-lg hover:bg-slate-800 hover:shadow-lg hover:shadow-slate-200 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
          >
            <Zap size={20} className="fill-white" /> 分配
          </button>
        </form>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {JARS_CONFIG.map((jar) => (
          <div
            key={jar.id}
            className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-lg hover:shadow-slate-100 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-[180px]"
          >
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full mt-1 inline-block">
                    {jar.percent}%
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="text-2xl font-black text-slate-800 tracking-tight">
                <span className="text-sm text-slate-400 mr-1 font-normal">
                  RM
                </span>

                {balances[jar.id].toLocaleString('en-US', {
      ...
      <div className="bg-white border border-violet-100 rounded-3xl p-8 shadow-xl shadow-violet-50 relative overflow-hidden">
        <div className="flex justify-between items-end mb-4 relative z-10">
          ...
          <div className="text-right">
            <div className="text-3xl font-black text-slate-800 tracking-tight">
              RM {currentSavings.toLocaleString()}{' '}
        ...
        <div className="h-6 bg-slate-100 rounded-full overflow-hidden relative">
          ...
          >
            ...
            {progressPercent > 5 && (
              <span className="text-[10px] text-white font-bold relative z-10">
                {progressValue}%
              </span>
            )}
          </div>
        ...
        <div className="mt-3 flex justify-between text-xs font-bold text-slate-400">
          ...
          <span className={progressPercent >= 100 ? 'text-violet-600' : ''}>
            {progressPercent >= 100 ? 'GOAL ACHIEVED' : 'IN PROGRESS'}
          </span>
...
const CycleTrackerView = ({
      ...
      prev.map((cycle) => {
        ...
        if (cycle.tasks.length <= 1) {
          return {
            ...cycle,
            tasks: cycle.tasks.map((t) =>
              t.id === taskId ? { ...t, text: '', done: false } : t
            ),
          };
        }
  ...
  const exportToExcel = () => {
    ...
    data.forEach((row) => {
      ...
      htmlContent += `<tr><td>${row.id}</td><td>${
        row.dateRange
      }</td><td>${taskList}</td><td>${row.actionPlan || ''}</td><td>${
        row.notes
      }</td></tr>`;
    });
    ...
    <div className="h-full flex flex-col animate-fade-in pb-20">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-4 mb-6 shadow-sm">
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-bold text-slate-400 tracking-widest uppercase flex items-center gap-2">
            <TrendingUp size={12} /> Year Progress
          </span>
        ...
        <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
          <div
            className="bg-gradient-to-r from-violet-500 to-blue-500 h-full rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(139,92,246,0.3)]"
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-1">
        ...
        {data.map((row) => (
          <div
            key={row.id}
            className="bg-white border border-slate-100 rounded-2xl p-5 md:grid md:grid-cols-12 md:gap-4 md:items-start hover:shadow-lg hover:shadow-slate-100 hover:-translate-y-[1px] transition-all group duration-300"
          >
            <div className="col-span-2 flex md:flex-col justify-between items-center md:items-start gap-2 mb-4 md:mb-0 border-b md:border-none border-slate-100 pb-3 md:pb-0">
              <div className="flex items-center gap-3">
                <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-violet-50 text-violet-600 font-bold text-sm shadow-sm">
                  {row.id}
                </span>

                <span className="md:hidden text-slate-500 text-sm font-medium">
                  {row.dateRange}
                </span>
              </div>

              <span className="hidden md:block text-slate-500 text-[11px] font-bold mt-1 bg-slate-50 px-2 py-1 rounded text-center w-full">
                {row.dateRange}
              </span>
            </div>
            ...
            <div className="col-span-5 space-y-3 mb-5 md:mb-0">
              {row.tasks.map((task) => (
                ...
                >
                  <button
                    onClick={() => toggleTask(row.id, task.id)}
                  ...
                  <input
                    ...
                    value={task.text}
                    onChange={(e) =>
                      handleTaskTextChange(row.id, task.id, e.target.value)
                    }
                    placeholder="Add task..."
                    className={`flex-1 bg-transparent border-b border-transparent hover:border-slate-100 focus:border-violet-300 outline-none text-sm py-1 transition-all font-medium ${
                      task.done
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover/task:opacity-100 text-slate-300 hover:text-red-400 transition-opacity"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
              ...
              <button
                ...
                className="text-[11px] text-violet-500 hover:text-violet-700 flex items-center gap-1 font-bold mt-2 bg-violet-50 hover:bg-violet-100 px-3 py-1.5 rounded-lg transition-colors w-max"
              >
            ...
            <div className="col-span-2">
              <span className="md:hidden text-xs text-slate-400 font-bold block mb-1">
                Action Plan:
              </span>
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleFieldChange(row.id, 'actionPlan', e.target.value)
                }
      ...
      <header className="flex justify-between items-end pb-4 border-b border-slate-100 md:border-none md:pb-0">
        <div>
          ...
          <p className="text-slate-500 font-medium text-lg">
            You have{' '}
            <span className="text-violet-600 font-bold">{pendingCount}</span>{' '}
            pending tasks today.
          </p>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-violet-600 to-indigo-600 p-8 rounded-[2rem] text-white shadow-xl shadow-violet-200 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="text-violet-100 text-xs font-bold uppercase mb-3 tracking-widest">
                Efficiency
              </div>
              ...
              <div className="flex items-end gap-2">
                <span className="text-5xl font-black tracking-tight">
                  {todaysTasks.length
                    ? Math.round(
                        ((todaysTasks.length - pendingCount) /
                          todaysTasks.length) *
                          100
                      )
                    : 0}
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-lg shadow-slate-100/50 h-full">
            ...
            <div className="space-y-4">
              {annualGoals.slice(0, 3).map((goal) => (
                <div
                  key={goal.id}
                  className="text-sm text-slate-600 flex items-start gap-3 group"
                >
                  <div
                    className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 transition-colors ${
                      goal.completed
                        ? 'bg-emerald-400'
                        : 'bg-slate-200 group-hover:bg-violet-400'
                    }`}
                  ></div>
    ...
    <div className="h-full flex flex-col animate-fade-in pb-20 md:pb-0">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm font-bold mt-1 uppercase tracking-wide">
            {year} •{' '}
        ...
        <div className="flex gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm">
          <button
            ...
            className="p-2 hover:bg-slate-50 rounded-xl transition text-slate-400 hover:text-slate-800"
          >
            <ChevronLeft size={20} />
          </button>
          ...
          <button
            ...
            className="px-4 text-xs font-bold text-violet-600 bg-violet-50 hover:bg-violet-100 rounded-xl transition"
          >
            TODAY
          </button>
          ...
          {totalSlots.map((day, i) => {
            ...
            return (
              <div
                ...
                className="bg-white p-3 hover:bg-violet-50/30 transition-colors cursor-pointer group relative flex flex-col min-h-[80px]"
              >
                ...
                <div className="space-y-1.5 overflow-hidden">
                  {dayTasks.slice(0, 3).map((t) => (
                    <div
                      key={t.id}
                      className={`text-[10px] px-2 py-1 rounded-md truncate font-medium flex items-center gap-1.5 border ${
                        t.completed
                          ? 'bg-slate-50 text-slate-400 border-transparent line-through'
                          : 'bg-white border-violet-100 text-slate-600 shadow-sm'
                      }`}
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${
                          t.completed ? 'bg-slate-300' : 'bg-violet-500'
                        }`}
                      ></div>
                  ...
                  {dayTasks.length > 3 && (
                    <div className="text-[10px] text-slate-400 pl-1 font-bold">
...
const BoardView = ({
  ...
  const weekDays = [...Array(7)].map(
    (_, i) => new Date(curr.setDate(first + i) && curr)
  );
        ...
        <div className="h-full flex gap-6 min-w-[1200px] pb-6 px-1">
          {weekDays.map((day, i) => {
            ...
            return (
              <div
                key={i}
                className={`flex-1 min-w-[280px] rounded-[2rem] flex flex-col transition-all ${
                  isToday
                    ? 'bg-violet-50/50 border-2 border-violet-100'
              ...
              >
                <div className="p-5 border-b border-slate-50 flex justify-between items-center rounded-t-[2rem]">
                  <div>
                    ...
                    <div
                      className={`text-2xl font-black ${
                        isToday ? 'text-violet-900' : 'text-slate-800'
                      }`}
                    >
                      {day.getDate()}
                    </div>
                  ...
                  <button
                    onClick={() => openAddModal(dateStr)}
                    className="w-8 h-8 rounded-full hover:bg-white text-slate-400 hover:text-violet-600 transition-all flex items-center justify-center shadow-sm border border-transparent hover:border-slate-100"
                  >
                    <Plus size={18} />
                  </button>
                </div>
...
const YearlyView = ({
  ...
  const yearlyProgress = yearlyTotal
    ? Math.round((yearlyCompleted / yearlyTotal) * 100)
    : 0;
      ...
      <div className="bg-slate-900 rounded-3xl p-10 mb-10 relative overflow-hidden shadow-2xl shadow-slate-200 flex-shrink-0">
        ...
        <div className="relative z-10 text-white">
          <div className="flex justify-between items-end mb-6">
            <div>
              <div className="text-violet-300 font-bold text-xs uppercase tracking-widest mb-2">
                Annual Execution
              </div>
          ...
          <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
            <div
              ...
              className="h-full bg-gradient-to-r from-violet-500 to-indigo-500 shadow-[0_0_20px_rgba(139,92,246,0.5)] transition-all duration-1000"
            ></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-8">
        {months.map((m) => {
          ...
          return (
            <div
              key={m}
              className={`bg-white rounded-[1.5rem] p-6 h-[280px] flex flex-col transition-shadow duration-300 border ${
                isSelected
                  ? 'border-violet-200 shadow-xl shadow-violet-100 ring-1 ring-violet-100'
            ...
            >
              <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-50">
                ...
                <button
                  onClick={() => addMonthlyGoal(currentYear, m)}
                  className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-50 text-slate-400 hover:text-violet-600 transition-colors"
                >
                  <Plus size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                {goals.map((g) => (
                  <div key={g.id} className="flex gap-3 group items-start">
                    <button
                      onClick={() => toggleMonthlyGoal(currentYear, m, g.id)}
                    ...
                    {g.isNew ? (
                      <input
                        autoFocus
                        className="bg-transparent border-b border-violet-300 text-slate-800 outline-none w-full text-sm font-medium pb-1"
                        value={g.text}
                        onChange={(e) =>
                          updateMonthlyGoalText(
                            currentYear,
                        ...
                        onKeyDown={(e) =>
                          handleMonthlyEnter(currentYear, m, g.id, e)
                        }
                      ...
                      <div className="flex-1 relative group/item">
                        <span
                          ...
                          className={`text-sm font-medium transition-colors cursor-text block ${
                            g.completed
                              ? 'text-slate-400 line-through'
                              : 'text-slate-600'
                          }`}
                        ...
                        <button
                          onClick={() =>
                            deleteMonthlyGoal(currentYear, m, g.id)
                          }
                          className="absolute right-0 opacity-0 group-hover/item:opacity-100 text-slate-400 hover:text-red-500 transition-opacity"
                        >
                          <X size={14} />
                        </button>
                      </div>
...
export default function App() {
  ...
  const [wealthBalances, setWealthBalances] = useState({
    commitment: 0,
    savings: 0,
    investment: 0,
    education: 0,
  ...
  useEffect(() => {
    if (user) {
      ...
      unsubs.push(
        onSnapshot(
          doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'tasks'),
          (doc) => {
      ...
      unsubs.push(
        onSnapshot(
          doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'goals'),
          (doc) => {
            if (doc.exists()) {
              const d = doc.data();
              setAnnualGoals(d.annual || []);
              setMonthlyGoals(d.monthly || {});
            }
          }
        )
      );
      ...
      unsubs.push(
        onSnapshot(
          doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'wealth'),
          (doc) => {
            if (doc.exists())
              setWealthBalances(
                doc.data().balances || {
      ...
      unsubs.push(
        onSnapshot(
          doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'cycles'),
          (doc) => {
            if (doc.exists()) {
              setCyclesData(doc.data().list || []);
              setStartYearDate(
                doc.data().startDate || new Date().getFullYear() + '-01-01'
              );
            } else {
              const initial = generateInitialCycles(
                new Date().getFullYear() + '-01-01'
              );
  ...
  const loadFromLocalStorage = () => {
    try {
      const t = localStorage.getItem('modern_tasks');
      setTasks(t ? JSON.parse(t) : []);

      const ag = localStorage.getItem('modern_annual_goals');
      setAnnualGoals(ag ? JSON.parse(ag) : []);

      const mg = localStorage.getItem('modern_monthly_goals');
      setMonthlyGoals(mg ? JSON.parse(mg) : {});
      ...
      setCyclesData(
        cd
          ? JSON.parse(cd)
          : generateInitialCycles(new Date().getFullYear() + '-01-01')
      );

      const sd = localStorage.getItem('tracker_startDate');
      if (sd) setStartYearDate(sd);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const saveData = (type, data) => {
    if (user) {
      ...
      if (type === 'goals')
        setDoc(
          doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'goals'),
          { annual: annualGoals, monthly: monthlyGoals }
        );
      ...
      if (type === 'cycles')
        setDoc(
          doc(db, 'artifacts', appId, 'users', user.uid, 'data', 'cycles'),
          { list: data, startDate: startYearDate }
        );
    } else {
      if (type === 'tasks')
        localStorage.setItem('modern_tasks', JSON.stringify(data));

      if (type === 'goals') {
        localStorage.setItem(
          'modern_annual_goals',
          JSON.stringify(annualGoals)
        );
        localStorage.setItem(
          'modern_monthly_goals',
          JSON.stringify(monthlyGoals)
        );
      }

      if (type === 'wealth')
        localStorage.setItem('wealth_balances', JSON.stringify(data));

      if (type === 'cycles') {
        localStorage.setItem('tracker_data', JSON.stringify(data));
        localStorage.setItem('tracker_startDate', startYearDate);
      }
    }
  };

  useEffect(() => {
    if (tasks.length > 0 || user) saveData('tasks', tasks);
  }, [tasks]);

  useEffect(() => {
    if (annualGoals.length > 0 || Object.keys(monthlyGoals).length > 0 || user)
      saveData('goals', null);
  }, [annualGoals, monthlyGoals]);

  useEffect(() => {
    saveData('wealth', wealthBalances);
  }, [wealthBalances]);

  useEffect(() => {
    if (cyclesData.length > 0 || user) saveData('cycles', cyclesData);
  }, [cyclesData, startYearDate]);
  ...
  const addMonthlyGoal = (year, month) => {
    ...
    setMonthlyGoals((prev) => {
      ...
      return {
        ...
        [key]: [
          ...current,
          { id: Date.now(), text: '', completed: false, isNew: true },
        ],
      };
    });
  };
          ...
          <nav className="space-y-1.5">
            {[
              ...
              >
                <item.icon
                  size={18}
                  className={view === item.id ? 'text-violet-300' : ''}
                />
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center justify-between">
            {user ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white py-2.5 rounded-xl font-bold text-xs hover:bg-slate-800 transition-colors"
              >
          ...
          <div className="bg-slate-900 rounded-2xl p-5 border border-slate-800 shadow-lg relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <div className="text-2xl font-black text-white tracking-tight">
                <span className="text-xs text-slate-500 mr-1 font-normal">
                  RM
                </span>
        ...
        <div className="flex-1 p-5 md:p-10 overflow-y-auto custom-scrollbar md:pb-10 relative">
          ...
          {view === 'yearly' && (
            <YearlyView
              ...
              toggleMonthlyGoal={toggleMonthlyGoal}
              updateMonthlyGoalText={updateMonthlyGoalText}
              handleMonthlyBlur={handleMonthlyBlur}
              handleMonthlyEnter={handleMonthlyEnter}
